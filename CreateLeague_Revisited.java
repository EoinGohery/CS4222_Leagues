...
public class CreateLeague
{
    ...
    private static String[] leagueMenu;
    private static int currentLeagueNum = 1;
    private static Scanner x;
    private static Scanner y;
    private static Scanner z;
    private static Scanner findAdminNum;
    private static Scanner findLeagueNum;
    ...
    private static File results = new File ("results.txt");
    
    public static void main(String[] args) throws IOException {
      login();
      establishAdmin(currentAdminNum);
      boolean main = true;
      while (main) {
          boolean sub = true;
          ...
          if(section=="Create League") {
              ...
              //reads through the league file and displays all league names of leagues belonging to currrentAdminNum. Add to leagueMenu String Array
              //String choosenLeague = (String) JOptionPane.showInputDialog(null, "Menu","",JOptionPane.QUESTION_MESSAGE, null, leagueMenu, leagueMenu[0]);
              //the league number is then found
            while (sub) {
    ...
    public static void createLeague() throws IOException {
        ...
        numberOfTeams = Integer.parseInt(JOptionPane.showInputDialog(null,leagueName + "\n How many teams are to be in the league? \n NOTE: If team number is odd the system will automatically add +1"));
        findLeagueNum = new Scanner(leagueInfo);
        while(findLeagueNum.hasNext()) {
            findLeagueNum.next();
            newLeagueNum++;
        }
        currentLeagueNum = newLeagueNum;
        ...
        for (int i = 0; i < numberOfTeams; i++) {
            ...
            fileTeamNames =currentLeagueNum + "," + (i + 1) + "," + leagueTeamNames[i]; // Actual format to be written to file
            pw2.println(fileTeamNames);
        }
        JOptionPane.showMessageDialog(null,teamNames); // List teams

        String LeagueDetails = currentLeagueNum + "," + leagueName + "," + currentAdminNum;
        ...
        if (accountInfo.length() == 0) {
            ...
            try { 
                ...
                while(x.hasNext() && !found) {
                    ...
                    tempUser = x.next();
                    tempPass = x.next();
                    currentAdminNum = Integer.parseInt(tempNum);
