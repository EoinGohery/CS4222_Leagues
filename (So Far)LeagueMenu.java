...
public class LeagueMenu
{
   ...
   public static void login() throws IOException{
	...
	public static void CheckLeagueName(String leagueName)
	{ check = new Scanner(leagueInfo)
		boolean exists;
		while(check.hasNext() && !exists) {
			tempNum = check.next();
			tempUser = check.next();
			tempPass = check.next();
			if(tempUser.trim().equals(league))
			{   Random r = new Random();
				...
				JOptionPane.showMessageDialog("This League name already exists,so your league will be called" leagueName + c + d + e);
				leagueName = leagueName + c + d + e;
				exists = true;
